...
════════════════════════════════════════════════════════════════════
  ...
  HOW TO RUN
════════════════════════════════════════════════════════════════════
  # 0. Start the MLflow server ONCE (leave it running in its own terminal).
  #    This is the same server the UI uses, so runs show up live — exactly
  #    like yolov8n_next_ver.py. Without this, training can't log.
  mlflow server --host 127.0.0.1 --port 5000 \
    --backend-store-uri sqlite:////home/smartan/weight_recognition/mlflow.db \
    --artifacts-destination /home/smartan/weight_recognition/mlartifacts

  # 1. Activate your venv
  source /home/pc-008/weight_recognition/venv/bin/activate

  # 2. Basic run — nano, GPU 0, all defaults (logs to the server above)
  ...
  # 6. Standalone file mode instead of the server (NOT recommended — this is
  #    what caused runs to "disappear": the file lands in the current folder)
  python train_v8n_db_fix.py --mlflow-uri sqlite:////home/smartan/weight_recognition/mlflow.db

  # 7. Resume interrupted run
  python train_v8n_db_fix.py --resume

  # 8. Compare this run vs the base v9n run in MLflow UI
  #    Just open the server you started in step 0:
  #    http://127.0.0.1:5000
...
def parse_args():
    ...
    p.add_argument("--name",     default="gym_equipment_v8n_db_fix_6")
    p.add_argument("--resume",   action="store_true")
    p.add_argument("--patience", type=int, default=50)
    p.add_argument("--data",     default=DATA_YAML)
    p.add_argument("--mlflow-uri",        default="http://127.0.0.1:5000",
                   help="MLflow tracking server (matches yolov8n_next_ver.py). "
                        "Start it with: mlflow server --host 127.0.0.1 --port 5000. "
                        "Override with a sqlite:///path for standalone file mode.")
...
def main():
    ...
    train_kwargs = dict(
        ...
        copy_paste=0.4,            # CHANGED from 0.4
        close_mosaic=15,
